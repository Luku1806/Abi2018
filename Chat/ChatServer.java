...
public class ChatServer {
	...
	public void start() {
		...
		// something went bad
			...
			String msg = stf.format(new Date()) + " Exception on new ServerSocket: " + e + "\n";
			display(msg);
		}
	}
	...
	private void display(String msg) {
		String event = stf.format(new Date()) + " " + msg;
		...
		public void run() {
			...
			while (keepGoing) {
				...
				case ChatMessage.WHOISIN:
					String usersListHead = "List of the users connected at " + stf.format(new Date()) + "\n";
					// scan al the users connected
					String usersList = "";
					for (int i = 0; i < al.size(); ++i) {
						ClientThread ct = al.get(i);
						usersList += ((i + 1) + ") " + ct.username + " since " + ct.date);
					}
					writeMsg(usersListHead + usersList);
					break;
				case ChatMessage.CHANGENAME:
					broadcast(username + " changed to " + message);
					username = message;
				}
